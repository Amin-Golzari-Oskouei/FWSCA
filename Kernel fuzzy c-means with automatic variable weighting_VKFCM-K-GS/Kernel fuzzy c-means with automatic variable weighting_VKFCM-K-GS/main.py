def main(data, center_points, k, t_max, row, fuzzy_degree, col, sigma, q):
    import numpy as np
    import math
    import sys
...
#-----------------------------------------------------------------------------------------
    ...
    while 1:
        ...
        E_w = object_fun(row, col, k, Cluster_elem, center_points, fuzzy_degree, data , z, sigma, q)
        ...
        tmp1 = np.zeros([col])
        for j in range(col):
            tmp2 = (dwm / (np.tile(dwm[j], (1, col)))) ** (1 / (q - 1))
            tmp2[np.where(np.isnan(tmp2))] = 0
            tmp2[np.where(np.isinf(tmp2))] = 0
            tmp1 = tmp1 + tmp2
