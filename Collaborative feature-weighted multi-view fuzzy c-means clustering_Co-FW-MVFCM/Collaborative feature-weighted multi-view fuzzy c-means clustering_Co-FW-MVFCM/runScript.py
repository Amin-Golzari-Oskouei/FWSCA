...
def runScript(data, k, t_max, row, fuzzy_degree, number_viwe, Restarts, lable_true, dataSetName):
    with open(f'{dataSetName}.txt', 'w') as file:
        file.write('Collaborative feature-weighted multi-view fuzzy c-means clustering_Co-FW-MVFCM\n\n')
    # a_coefficient, b_coefficient, balance_tarm
        for b in np.arange(0.1, 5.1, 0.3):
            landa = round(b, 1)
