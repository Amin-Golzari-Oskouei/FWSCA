...
for repeat in range(Restarts):
    ...
    w = return_weighted_distance(data)
    w = np.ones(col)
    fcm = c_means(c = k, center_points=center_points, m = fuzzy_degree,  max_iter = t_max, threshold = 10**-6)
    fcm.fit(data, w)
    end_time = time.time()
    RunTime_repeat[repeat] = end_time - start_time
    
    
    Cluster_elem = np.transpose(fcm.fuzzy_partition)
