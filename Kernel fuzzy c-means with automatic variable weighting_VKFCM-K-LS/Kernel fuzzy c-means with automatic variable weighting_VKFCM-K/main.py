def main(data, center_points, k, t_max, row, fuzzy_degree, col, q, sigma):
    import numpy as np
    import math
    import sys
...
#-----------------------------------------------------------------------------------------
    ...
    while 1:
        ...
        E_w = object_fun(row, col, k, Cluster_elem, center_points, fuzzy_degree, z, q, data, sigma)
        ...
        for j in range(k):
            ...
            center_points[j, :] = np.matmul(np.expand_dims(mf[j, :], 0), (data * tmp)) / np.matmul(np.expand_dims(mf[j, :], 0), tmp)
