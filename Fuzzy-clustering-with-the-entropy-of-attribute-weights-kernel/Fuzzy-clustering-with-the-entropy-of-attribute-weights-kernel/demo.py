# This demo shows how to call the extension of fuzzy C-means algorithm
...
for dataset in datasets:
    ...
    k, t_max, Restarts, fuzzy_degree, landa, sigma = parameters(dataset, lable_true, col, data)
    ...
    for repeat in range(Restarts):
        ...
        Cluster_elem = main(data, center_points, k, t_max, row, fuzzy_degree, col, landa, sigma)
        end_time = time.time()
        ...
        if Cluster_elem is not None and len(np.unique(lable_pre)) == k:
            ...
            print(f'The accuracy score in {repeat+1}th Restart is {ans[0]}.')
            ...
            print(f'The accuracy score in {repeat+1}th Restart is NaN.')
    ...
    print(f'Average accuracy score over {Restarts} restarts: {np.nanmean(ACC_repeat)}')
