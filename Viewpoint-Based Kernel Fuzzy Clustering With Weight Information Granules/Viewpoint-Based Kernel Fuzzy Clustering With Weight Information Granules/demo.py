...
for dataset in datasets:
    ...
    k, t_max, Restarts, fuzzy_degree, landa, sigma = parameters(dataset, lable_true)
    ...
    for repeat in range(Restarts):
        ...
        center_points, viewpoint = center_Points(data, sigma, k, col, row)
        ...
        for j in range(k):
            distance [:, j] = np.sum((2 * (np.exp(1) ** (((-1) * ((data - np.tile(center_points[j, :], (row, 1))) ** 2)) / (2 * (sigma ** 2))))), 1)

        beta_n = np.sum((distance), 1) / k

        # Execute proposed algorithm.
        # Get the cluster assignments and other parameters.

        start_time = time.time()
        Cluster_elem = main(data, center_points, k, t_max, row, fuzzy_degree, col, landa, sigma, beta_n, viewpoint)
        end_time = time.time()
