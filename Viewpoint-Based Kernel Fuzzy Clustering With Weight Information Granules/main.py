def main(data, center_points, k, t_max, row, fuzzy_degree, col , landa, sigma, beta_n, viewpoint):
    import numpy as np
    import math
    import sys
...
#-----------------------------------------------------------------------------------------
    ...
    while 1:
        ...
        E_w = object_fun(row, col, k, Cluster_elem, center_points, fuzzy_degree, data , landa , z, sigma, beta_n)
        ...
        mf = np.tile(beta_n, (k,1)) * (Cluster_elem ** fuzzy_degree)
        for j in range(k):
        ...
        tmp5 = np.sum(2 - (2 * (np.exp(1) ** (((-1) * (center_points - np.tile(viewpoint, (k, 1))) ** 2) / (2 * (sigma ** 2))))), 1)
        center_points[np.argmin(tmp5), :] = viewpoint
        
               
        # # Update the feature weights.
        for j in range(k):
            distance = 2 - (2 * (np.exp(1) ** (((-1) * ((data - np.tile(center_points[j, :], (row, 1))) ** 2)) / (2 * (sigma ** 2)))))
            dwkm[j,:] = np.exp((-1 * landa) * np.matmul(mf[j, :] ** fuzzy_degree, distance))
